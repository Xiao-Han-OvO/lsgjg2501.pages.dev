//#region node_modules/pinyin-pro/dist/index.mjs
...
var FastDictFactory = class {
	...
	get(word) {
		if (word.length > 1) return this.StringDICT.get(word);
		else {
	...
	set(word, pinyin) {
		if (word.length > 1) this.StringDICT.set(word, pinyin);
		else {
			const code = word.charCodeAt(0);
...
Object.keys(map).forEach((key) => {
	const chars = map[key];
	for (let char of chars) DICT1.set(char, key);
});
var InitialList = [
	"zh",
	"ch",
...
function genNumberDict() {
	...
	for (let number in Numbers) for (let key in NumberWordMap) {
		const word = `${number}${key}`;
		dict[word] = `${Numbers[number]} ${NumberWordMap[key]}`;
	}
	return dict;
}
...
function processToneSandhi(cur, pre, next) {
	if (toneSandhiList.indexOf(cur) === -1) return getSingleWordPinyin(cur);
	if (pre === next && pre && getSingleWordPinyin(pre) !== pre) return getPinyinWithoutTone(getSingleWordPinyin(cur));
	if (next && !toneSandhiIgnoreSuffix[cur].includes(next)) {
		...
		if (nextPinyin !== next) {
			...
			for (let pinyin in pinyinMap) if (pinyinMap[pinyin].indexOf(Number(nextTone)) !== -1) return pinyin;
		}
	}
}
function processToneSandhiLiao(cur, pre) {
	if (cur === "了" && (!pre || !DICT1.get(pre))) return "liǎo";
}
function processReduplicationChar(cur, pre) {
	if (cur === "々") if (!pre || !DICT1.get(pre)) return "tóng";
	else return DICT1.get(pre).split(" ")[0];
}
...
function getMaxProbability(a, b) {
	...
	else if (a.decimal === b.decimal) return a.probability > b.probability ? a : b;
	else return b;
}
...
function getPatternDecimal(pattern) {
	if (pattern.priority === Priority.Custom) return -(pattern.length * pattern.length * 100);
	if (pattern.priority === Priority.Surname) return -(pattern.length * pattern.length * 10);
	return 0;
}
...
function getPatternCount(pattern) {
	if (pattern.priority === Priority.Custom) return -(pattern.length * pattern.length * 1e5);
	if (pattern.priority === Priority.Surname) return -(pattern.length * pattern.length * 100);
	return 1;
}
...
function reverseMaxMatch(patterns) {
	...
	for (let i = patterns.length - 1; i >= 0;) {
		...
		let j = i - 1;
		while (j >= 0 && isIgnorablePattern(patterns[i], patterns[j])) j--;
		if (j < 0 || patterns[j].index + patterns[j].length <= index) filteredArr.push(patterns[i]);
		i = j;
	}
...
var AC = class {
	...
	buildFailPointer() {
		...
		while (queue.length > queueIndex) {
			...
			let key = node.key;
			while (failNode && !failNode.children.has(key)) failNode = failNode.fail;
			if (!failNode) node.fail = this.root;
			else node.fail = failNode.children.get(key);
		}
	}
	addPatternToDictMap(pattern) {
		if (!this.dictMap.has(pattern.dict)) this.dictMap.set(pattern.dict, /* @__PURE__ */ new Set());
		this.dictMap.get(pattern.dict).add(pattern);
	}
	addNodeToQueues(trieNode) {
		if (!this.queues[stringLength(trieNode.prefix)]) this.queues[stringLength(trieNode.prefix)] = [];
		this.queues[stringLength(trieNode.prefix)].push(trieNode);
	}
	insertPattern(patterns, pattern) {
		for (let i = patterns.length - 1; i >= 0; i--) {
			const _pattern = patterns[i];
			if (pattern.priority === _pattern.priority && pattern.probability >= _pattern.probability) patterns[i + 1] = _pattern;
			else if (pattern.priority > _pattern.priority) patterns[i + 1] = _pattern;
			else {
	...
	removeDict(dictName) {
		if (this.dictMap.has(dictName)) {
			this.dictMap.get(dictName).forEach((pattern) => {
	...
	match(text, surname) {
		...
		for (let i = 0; i < zhChars.length; i++) {
			let c = zhChars[i];
			while (cur !== null && !cur.children.has(c)) cur = cur.fail;
			if (cur === null) cur = this.root;
			else {
				cur = cur.children.get(c);
				const pattern = cur.patterns.find((item) => {
					if (surname === "off") return item.priority !== Priority.Surname;
					else if (surname === "head") return item.length - 1 - i === 0;
					else return true;
				});
				if (pattern) result.push(Object.assign(Object.assign({}, pattern), { index: i - pattern.length + 1 }));
	...
	search(text, surname, algorithm = 2) {
		...
		if (algorithm === 1) return reverseMaxMatch(patterns);
		else if (algorithm === 3) return minTokenization(patterns, stringLength(text));
...
function customPinyin(config = {}, options) {
	Object.keys(config).sort((word1, word2) => stringLength(word2) - stringLength(word1)).forEach((word) => {
	...
	if (options === null || options === void 0 ? void 0 : options.multiple) addCustomConfigToDict(config, customMultipleDict, options.multiple);
	if (options === null || options === void 0 ? void 0 : options.polyphonic) addCustomConfigToDict(config, customPolyphonicDict, options.polyphonic);
}
function addCustomConfigToDict(config, dict, handleType) {
	for (let word in config) {
		...
		splitString(word).forEach((char, index) => {
			...
			if (handleType === "replace" || handleType === "add" && !dict.get(char) && !DICT1.get(char)) dict.set(char, pinyin);
			else {
				dict.set(char, dict.get(char) || DICT1.get(char));
				if (!dict.get(char).split(" ").includes(pinyin)) dict.set(char, `${dict.get(char)} ${pinyin}`.trim());
			}
		});
	}
}
...
function clearCustomDict(dict) {
	...
	if (dict === "polyphonic" || dict.indexOf("polyphonic") !== -1) customPolyphonicDict.clear();
}
var traditionalDict = [];
...
var getTraditionalWords = (word) => {
	...
	for (let i = 0; i < word.length; i++) {
		const key = word[i];
		const code = key.charCodeAt(0);
		if (traditionalDict[code]) traditionalWords[i] = traditionalDict[code];
		else traditionalWords[i] = key;
	}
...
var getPinyin = (word, list, surname, segmentit, traditional) => {
	...
	for (let i = 0; i < zhChars.length;) {
		...
		if (match && i === match.index) {
			...
			let pinyinIndex = 0;
			if (traditional) match.zh = zhChars.slice(match.index, match.index + match.length).join("");
...
var getPinyinWithoutTone = (pinyin) => {
...
var getAllPinyin = (char, surname = "off") => {
	...
	if (customMultpileDict.get(char)) pinyin = customMultpileDict.get(char).split(" ");
	else if (surname !== "off") {
		const surnamePinyin = Surnames[char];
		if (surnamePinyin) pinyin = [surnamePinyin].concat(pinyin.filter((py) => py !== surnamePinyin));
	}
	return pinyin;
};
var getMultiplePinyin = (word, surname = "off") => {
	let pinyin = getAllPinyin(word, surname);
	if (pinyin.length > 0) return pinyin.map((value) => ({
		origin: word,
		result: value,
		isZh: true,
		originPinyin: value
...
var getInitialAndFinal = (pinyin, initialPattern) => {
	const pinyin_arr = pinyin.split(" ");
	const initial_arr = [];
	const final_arr = [];
	for (let _pinyin of pinyin_arr) for (let _initial of InitialList) if (_pinyin.startsWith(_initial)) {
		let _final = _pinyin.slice(_initial.length);
		if (SpecialInitialList.indexOf(_initial) !== -1 && SpecialFinalList.indexOf(_final) !== -1) _final = SpecialFinalMap[_final];
		initial_arr.push(_initial);
		final_arr.push(_final);
		break;
	}
	if (initialPattern === "standard") initial_arr.forEach((initial, index) => {
		if (initial === "y" || initial === "w") initial_arr[index] = "";
	});
	return {
...
var getNumOfTone = (pinyin) => {
	...
	const tone_num_arr = [];
	pinyin.split(" ").forEach((_pinyin) => {
		if (reg_tone1.test(_pinyin)) tone_num_arr.push("1");
		else if (reg_tone2.test(_pinyin)) tone_num_arr.push("2");
		else if (reg_tone3.test(_pinyin)) tone_num_arr.push("3");
		else if (reg_tone4.test(_pinyin)) tone_num_arr.push("4");
		else if (reg_tone0.test(_pinyin)) tone_num_arr.push("0");
		else if (special_tone.test(_pinyin)) tone_num_arr.push("0");
		else tone_num_arr.push("");
	});
	return tone_num_arr.join(" ");
};
var getPinyinWithNum = (pinyin, originPinyin) => {
...
var getFirstLetter = (pinyin, isZh) => {
	const first_letter_arr = [];
	pinyin.split(" ").forEach((pinyin) => {
...
function isNonZhScope(char, scope) {
	if (scope instanceof RegExp) return scope.test(char);
	return true;
}
var middleWareNonZh = (list, options) => {
	let nonZh = options.nonZh;
	if (nonZh === "removed") return list.filter((item) => item.isZh || !isNonZhScope(item.origin, options.nonZhScope));
...
var middlewareMultiple = (word, options) => {
	if (stringLength(word) === 1 && options.multiple) return getMultiplePinyin(word, options.surname);
	else return false;
};
...
var middlewareToneType = (list, options) => {
	switch (options.toneType) {
		...
		case "num":
			list.forEach((item) => {
				if (item.isZh) item.result = getPinyinWithNum(item.result, item.originPinyin);
			});
			break;
	}
};
var middlewareV = (list, options) => {
	if (options.v) list.forEach((item) => {
		if (item.isZh) item.result = item.result.replace(/ü/g, typeof options.v === "string" ? options.v : "v");
	});
};
var middlewareType = (list, options, word) => {
	...
	if (options.type === "array") return list.map((item) => item.result);
	if (options.type === "all") return list.map((item) => {
		...
		let polyphonic = [];
		if (pinyin !== "") polyphonic = [pinyin].concat(getAllPinyin(item.origin, options.surname).filter((item) => item !== pinyin));
		return {
			origin: item.origin,
			pinyin,
			initial,
...
var middlewareToneSandhi = (list, toneSandhi) => {
	if (toneSandhi === false) list.forEach((item) => {
		if (item.origin === "一") item.result = item.originPinyin = "yī";
		else if (item.origin === "不") item.result = item.originPinyin = "bù";
	});
	return list;
};
...
function pinyin(word, options) {
	...
	if (!validateType(word)) return word;
	if (word === "") return options.type === "array" || options.type === "all" ? [] : "";
	if (options.surname === void 0) if (options.mode === "surname") options.surname = "all";
	else options.surname = "off";
	if (options.type === "all") options.pattern = "pinyin";
	if (options.pattern === "num") options.toneType = "none";
	if (options.removeNonZh) options.nonZh = "removed";
	let { list } = getPinyin(word, Array(stringLength(word)), options.surname, options.segmentit, options.traditional);
	list = middlewareToneSandhi(list, options.toneSandhi);
	list = middleWareNonZh(list, options);
	if (middlewareMultiple(word, options)) list = middlewareMultiple(word, options);
...
function addDict(dict, options) {
	const patterns = [];
	const dictName = (typeof options === "object" ? options.name : options) || DefaultName;
	...
	for (let word in dict) {
		...
		if (wordLength === 1) addToOriginDict(dictName, word, pinyin, dict1Handle);
		if (Array.isArray(value)) patterns.push({
			zh: word,
			pinyin,
...
function addToOriginDict(dict, char, pinyin, handle = "add") {
	if (!originDictMap.get(dict)) originDictMap.set(dict, {});
	const originDict = originDictMap.get(dict);
	if (!originDict[char]) originDict[char] = DICT1.get(char);
	if (handle === "add") {
		const existedPinyin = DICT1.get(char);
		if (existedPinyin && !existedPinyin.split(" ").includes(pinyin)) DICT1.set(char, `${existedPinyin} ${pinyin}`);
		else if (!DICT1.get(char)) DICT1.set(char, pinyin);
	} else if (handle === "replace") DICT1.set(char, pinyin);
}
...
*/
var match = (text, pinyin, options) => {
	if ((options === null || options === void 0 ? void 0 : options.precision) === "any") options.lastPrecision = "any";
	if (options === null || options === void 0 ? void 0 : options.v) pinyin = pinyin.replace(/ü/g, "v");
	...
	if (completeOptions.space === "ignore") pinyin = pinyin.replace(/\s/g, "");
...
var getMatchLength = (pinyin1, pinyin2) => {
	let length = 0;
	for (let i = 0; i < pinyin1.length; i++) if (pinyin1[i] === pinyin2[length]) length++;
	return length;
};
var matchAny = (text, pinyin$1, options) => {
	...
	if (pinyin$1) return null;
	if (options.continuous) {
		const _result = result;
		if (result.some((val, index) => index > 0 && val !== _result[index - 1] + 1)) return null;
	}
	if (options.space === "ignore") result = result.filter((i) => words[i] !== " ");
...
var matchAboveStart = (text, pinyin$1, options) => {
	...
	for (let i = 0; i < dp[0].length; i++) dp[0][i] = [];
	for (let i = 1; i < dp.length; i++) {
		if (!options.continuous || options.space == "ignore" && words[i - 1] === " ") for (let j = 1; j <= pinyin$1.length; j++) dp[i][j - 1] = dp[i - 1][j - 1];
		for (let j = 1; j <= pinyin$1.length; j++) if (!dp[i - 1][j - 1]) continue;
		else if (j !== 1 && !dp[i - 1][j - 1].length) continue;
		else {
			...
			if (words[i - 1] === pinyin$1[j - 1]) {
				...
				if (!dp[i][j] || matches.length > dp[i][j].length) dp[i][j] = matches;
				if (j === pinyin$1.length) return dp[i][j];
			}
			if (pinyin$1.length - j <= MAX_PINYIN_LENGTH) {
				if (muls.some((py) => {
					if (options.lastPrecision === "any") return py.includes(pinyin$1.slice(j - 1, pinyin$1.length));
					if (options.lastPrecision === "start") return py.startsWith(pinyin$1.slice(j - 1, pinyin$1.length));
					if (options.lastPrecision === "first") return py[0] === pinyin$1.slice(j - 1, pinyin$1.length);
					if (options.lastPrecision === "every") return py === pinyin$1.slice(j - 1, pinyin$1.length);
					return false;
				})) return [...dp[i - 1][j - 1], i - 1];
			}
			const precision = options.precision;
			if (precision === "start") muls.forEach((py) => {
				let end = j;
				const matches = [...dp[i - 1][j - 1], i - 1];
				while (end <= pinyin$1.length && py.startsWith(pinyin$1.slice(j - 1, end))) {
					if (!dp[i][end] || matches.length > dp[i][end].length) dp[i][end] = matches;
					end++;
				}
			});
			if (precision === "first") {
			...
			if (completeMatch) {
				...
				if (!dp[i][endIndex] || matches.length > dp[i][endIndex].length) dp[i][endIndex] = matches;
			}
		}
	}
	return null;
...
function processDoubleUnicodeIndex(text, indexArray) {
	...
	for (let j = 0; j < indexArray.length; j++) {
		...
		while (i <= curIndex) {
			if (words[i].length === 2) doubleUnicodeCount++;
			i++;
		}
		const realIndex = curIndex + doubleUnicodeCount;
		if (words[curIndex].length === 2) result.push(realIndex - 1, realIndex);
		else result.push(realIndex);
	}
	return result;
}
...
var html = (text, options) => {
	...
	return pinyin(text, Object.assign({ type: "all" }, completeOptions)).map((item) => {
		let additionalClass = "";
		for (const classname in completeOptions.customClassMap) if (completeOptions.customClassMap[classname].includes(item.origin)) additionalClass += ` ${classname}`;
		if (item.isZh) {
		...
		} else if (completeOptions.wrapNonChinese) return `<span class="${completeOptions.nonChineseClass || DefaultHtmlOptions.nonChineseClass}${additionalClass}">${item.origin}</span>`;
		else return item.origin;
	}).join("");
};
...
function polyphonic(text, options = DEFAULT_OPTIONS$1) {
	...
	return doubleList.map((list) => handleType(list, options));
}
var getPolyphonicList = (text) => {
	return splitString(text).map((char) => {
		const pinyin = getCustomPolyphonicDict().get(char) || DICT1.get(char) || char;
		return {
			origin: char,
			result: pinyin,
			isZh: pinyin !== char,
...
var handleType = (list, options) => {
	if (options.type === "array") return Array.from(new Set(list.map((item) => item.result)));
	if (options.type === "all") return list.map((item) => {
		const pinyin = item.isZh ? item.result : "";
		const { initial, final } = getInitialAndFinal(pinyin, options.initialPattern);
		const { head, body, tail } = getFinalParts(pinyin);
...
function convert(pinyin, options) {
	...
	if (typeof pinyin === "string") pinyin = pinyin.split(options.separator);
	pinyin = pinyin.map((item) => {
		const format = options.format;
		if (format === "numToSymbol") return formatNumToSymbol(item);
		else if (format === "symbolToNum") return formatSymbolToNum(item);
		else if (format === "toneNone") return formatToneNone(item);
		return item;
	});
	if (originType === "string") return pinyin.join(options.separator);
	else return pinyin;
}
function formatNumToSymbol(pinyin) {
	const lastChar = Number(pinyin[pinyin.length - 1]);
	if (lastChar >= 0 && lastChar <= 4) {
		for (let key in toneMap) if (pinyin.includes(key)) return pinyin.slice(0, pinyin.length - 1).replace(key, toneMap[key][lastChar]);
		return pinyin;
	} else return pinyin;
}
...
function middlewareSegment(list, matches) {
	...
	while (i < list.length && j < matches.length) {
		...
		if (match.zh.startsWith(item.origin)) {
			...
			let cur = start + 1;
			while (cur < list.length && list[cur].origin === chars[cur - start]) cur++;
...
function middlewareOutputFormat(segments, options) {
	...
	if (format === OutputFormat.AllSegment) return segments.map((item) => {
		return {
			origin: item.segment.map((item) => item.origin).join(""),
			result: item.segment.map((item) => item.result).join("")
		};
	});
	else if (format === OutputFormat.AllArray) return segments.map((item) => item.segment);
	...
	} else if (format === OutputFormat.PinyinSegment) return segments.map((item) => item.segment.map((item) => item.result).join(""));
	else if (format === OutputFormat.PinyinArray) return segments.map((item) => item.segment.map((item) => item.result));
	else if (format === OutputFormat.PinyinString) return segments.map((item) => item.segment.map((item) => item.result).join("")).join(separator);
	else if (format === OutputFormat.ZhSegment) return segments.map((item) => item.segment.map((item) => item.origin).join(""));
	else if (format === OutputFormat.ZhArray) return segments.map((item) => item.segment.map((item) => item.origin));
	else if (format === OutputFormat.ZhString) return segments.map((item) => item.segment.map((item) => item.origin).join("")).join(separator);
}
var DEFAULT_OPTIONS = {
	toneType: "symbol",
	mode: "normal",
...
function segment(word, options) {
	...
	let { list, matches } = getPinyin(word, Array(stringLength(word)), options.surname, options.segmentit, options.traditional);
	list = middlewareToneSandhi(list, options.toneSandhi);
	list = middleWareNonZh(list, options);
	middlewareToneType(list, options);
	middlewareV(list, options);
	return middlewareOutputFormat(middlewareSegment(list, matches), {
...
//#endregion
export { OutputFormat, addDict, addTraditionalDict, clearCustomDict, convert, customPinyin, getFinalParts, getInitialAndFinal, getNumOfTone, getTraditionalDict, html, match, pinyin, polyphonic, removeDict, segment };
